...
void PrepareOpenCLFarField(const doublecomplex *polarization,const int shifted_index)
/* Select the cheapest valid device view of the physical polarization. For solver/formulation combinations where no
 * such view exists, defer one upload of the canonical host pvec until the first direct far-field batch.
 */
{
...
#ifdef OCL_BLAS
	if (IterMethod==IT_SHIFTED_BICG_CS && shifted_index>=0) {
		if (shifted_index>=num_used_n) LogError(ONE_POS,"Invalid shifted-system index in PrepareOpenCLFarField");
		polarization_buffer=bufxArray;
		polarization_offset=(size_t)shifted_index*local_nRows;
		return;
	}
	if (IterMethod==IT_BICG_CS && MatVecMode==MV_STANDARD) {
		polarization_buffer=bufxvec;
		return;
	}
#else
	(void)shifted_index;
...
// upload pvec only when the completed solver does not already provide the physical polarization in a device buffer
{
	if (!polarization_prepared) PrepareOpenCLFarField(pvec,-1);

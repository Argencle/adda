...
#define EPS1 1E-30
	...
	case PHASE_INIT: {
		...
		CLBLAS_CH_ERR(clblasZscal(local_nRows,clinvpn,bufvcur,0,1,1,&command_queue,0,NULL,
			PROFILE_LA_OPENCL_EVENT(PROF_LA_OPENCL_ZSCAL)));
#else
		...
		CL_CH_ERR(clEnqueueNDRangeKernel(command_queue,clzero,1,NULL,&shifted_vec_rows,NULL,0,NULL,
			PROFILE_LA_OPENCL_EVENT(PROF_LA_OPENCL_ZERO)));
		CL_CH_ERR(clSetKernelArg(clzero,0,sizeof(cl_mem),&bufpArray));
		CL_CH_ERR(clEnqueueNDRangeKernel(command_queue,clzero,1,NULL,&shifted_array_rows,NULL,0,NULL,
			PROFILE_LA_OPENCL_EVENT(PROF_LA_OPENCL_ZERO)));
	...
	case PHASE_ITER:
		...
		CLBLAS_CH_ERR(clblasZdotu(local_nRows,bufdot,0,bufvcur,0,1,bufAvecbuffer,0,1,buftmp,1,&command_queue,0,
			NULL,PROFILE_LA_OPENCL_EVENT(PROF_LA_OPENCL_ZDOTU)));
...
#ifdef OCL_BLAS
		CLBLAS_CH_ERR(clblasZdotc(local_nRows,bufdot,0,bufvtmp,0,1,bufvtmp,0,1,buftmp,1,&command_queue,0,NULL,
			PROFILE_LA_OPENCL_EVENT(PROF_LA_OPENCL_ZDOTC)));
...
#ifdef OCL_BLAS
		CL_CH_ERR(clEnqueueCopyBuffer(command_queue,bufvcur,bufvpr,0,0,sizeof(doublecomplex)*local_nRows,0,NULL,
			PROFILE_LA_OPENCL_EVENT(PROF_LA_OPENCL_COPY)));
		CL_CH_ERR(clEnqueueCopyBuffer(command_queue,bufvnext,bufvcur,0,0,sizeof(doublecomplex)*local_nRows,0,NULL,
			PROFILE_LA_OPENCL_EVENT(PROF_LA_OPENCL_COPY)));
		CL_CH_ERR(clFinish(command_queue));
		my_clReleaseBuffer(bufdot);
#else
